...
// general handler to attach panning
// behavior to globe
module.exports = (svg, projection, path, render) => {
  var gpos0;
  ...
  const straight = window.straight = () => {
  ...
  const zoom = window.zoom = num => {
      var s = projection.scale();
      projection.scale(s + num);
      render();
  };

  const horiz = window.horiz = num => {
      const r = projection.rotate();
      r[0] += num;
      projection.rotate(r);
      render();
  };
  const vert = window.vert = num => {
      const r = projection.rotate();
      r[1] += num;
      projection.rotate(r);
      render();
  };
  const roll = window.roll = num => {
      const r = projection.rotate();
      r[2] += num;
      projection.rotate(r);
      render();
